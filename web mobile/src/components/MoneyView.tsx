...
export default function MoneyView({
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      // Simulate real bank connection syncing values
      setAccounts(prev => prev.map(acc => ({
        ...acc,
        balance: acc.balance + (Math.random() > 0.5 ? 50 : -20)
      })));
  ...
  const submitSignature = () => {
    ...
    setTimeout(() => {
      setDocuments(prev => prev.map(doc => doc.id === selectedDocToSign?.id ? { ...doc, status: 'Проведен' } : doc));
      
      // Withdraw corresponding money
      if (selectedDocToSign) {
        setAccounts(prev => prev.map(acc => {
          if (acc.currency === selectedDocToSign.currency) {
            // Subtract debit correctly
            return { ...acc, balance: Math.max(0, acc.balance - selectedDocToSign.amount) };
          }
          return acc;
        }));
      }
  ...
  const handleVerifyTaxId = (e: React.FormEvent) => {
    e.preventDefault();
    if (!unpInput) return;
            ...
            <div className="divide-y divide-gray-100">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-xs font-extrabold text-gray-805 leading-none">Сотрудники компании</div>
