...
export default function CreateDocumentModal({
  ...
  // Submit generic document into state
  const handleAddNewDocument = (type: string, counterparty: string, docAmount: number, docCurrency: 'BYN' | 'USD' | 'EUR' | 'RUB', docPurpose: string) => {
    // Check balances
    const selectedAcc = accounts.find(a => a.id === sourceAcc);
    if (selectedAcc && selectedAcc.balance < docAmount) {
      alert(`Недостаточно средств на выбранном счете для совершения перевода. Доступно: ${selectedAcc.balance.toLocaleString()} ${selectedAcc.currency}`);
      return false;
    }

    // Deduct balance (mock simulation)
    setAccounts(prev => prev.map(a => {
      if (a.id === sourceAcc) {
        return { ...a, balance: Math.max(0, a.balance - docAmount) };
      }
      return a;
    }));

    const newDocId = `№ ${Math.floor(110 + Math.random() * 880)}`;
    ...
    alert(`${type} успешно сформирован как черновик и сохранен! Вы можете подписать его в разделе документов.`);
    handleCloseAll();
    return true;
  };
