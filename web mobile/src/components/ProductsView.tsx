  ...
  return (
    <div className="space-y-6 font-sans select-none">
      ...
      <div>
        ...
        <p className="text-xs text-gray-400 mt-1">Оформление и управление банковскими услугами и инструментами ОАО «Сбер Банк»</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-150 p-6 shadow-xs flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-3 border-b pb-3.5 mb-4">
              <div className="p-2.5 bg-emerald-50 rounded-xl text-sber-green">
            ...
            <div className="space-y-3.5 text-xs">
              ...
              <button
                onClick={() => alert('Запрос на новую карту Visa / MasterCard Business отправлен в операционный центр ОАО «Сбер Банк» Belarus.')}
                disabled={!can('manage_products')}
                title={can('manage_products') ? undefined : denyTitle('manage_products')}
                className="w-full text-left flex items-center justify-between text-sky-700 hover:underline font-semibold disabled:text-gray-300 disabled:no-underline disabled:cursor-not-allowed"
              >
              ...
              <button
                onClick={() => alert('Выписка доверенности на сотрудников для получения пластиковых карт в кассах Сбера.')}
                disabled={!can('manage_products')}
                title={can('manage_products') ? undefined : denyTitle('manage_products')}
                className="w-full text-left flex items-center justify-between text-sky-700 hover:underline font-semibold disabled:text-gray-300 disabled:no-underline disabled:cursor-not-allowed"
              >
              ...
              <button
                onClick={() => setActiveTool('card_mgmt')}
                disabled={!can('manage_products')}
                title={can('manage_products') ? undefined : denyTitle('manage_products')}
                className="w-full text-left flex items-center justify-between text-sky-700 hover:underline font-semibold disabled:text-gray-300 disabled:no-underline disabled:cursor-not-allowed"
              >
                <span className="flex items-center gap-1">Управление картами <span className="bg-emerald-600 text-white text-[9px] font-bold px-1 rounded animate-pulse">Настройки</span></span>
                <ChevronRight className="w-3.5 h-3.5 text-gray-300" />
              </button>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-150 p-6 shadow-xs flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-3.5 text-xs">
              <button
                onClick={() => alert('Открытие краткосрочных депозитных счетов в BYN / CNY. Пожалуйста, воспользуйтесь Калькулятором сначала.')}
              ...
              <button
                onClick={() => alert('Условия досрочного пополнения депозитного тела.')}
                disabled={!can('manage_products')}
                title={can('manage_products') ? undefined : denyTitle('manage_products')}
                className="w-full text-left flex items-center justify-between text-sky-700 hover:underline font-semibold disabled:text-gray-300 disabled:no-underline disabled:cursor-not-allowed"
              >
              ...
              <button onClick={() => setActiveTool('calculator')} className="w-full text-left flex items-center justify-between text-sky-700 hover:underline font-semibold" title="Калькулятор доступен всем ролям">
                <span className="flex items-center gap-1.5 text-teal-800 font-bold bg-teal-50 px-2 py-0.5 rounded border border-teal-150">
                  <Sliders className="w-3 h-3 text-sber-green" />
        ...
        <div className="bg-white rounded-2xl border border-gray-150 p-6 shadow-xs flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-3.5 text-xs">
              ...
              <button
                onClick={() => setActiveTool('open_account')}
                disabled={!can('open_account')}
                title={can('open_account') ? undefined : denyTitle('open_account')}
                className="w-full text-left flex items-center justify-between text-sky-700 hover:underline font-semibold disabled:text-gray-300 disabled:no-underline disabled:cursor-not-allowed"
              >
        ...
        <div className="bg-white rounded-2xl border border-gray-150 p-6 shadow-xs flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-3.5 text-xs">
              <button
                onClick={() => alert('Программа кредитования «Развитие»: субсидируемая ставка Сбера от 7% годовых в BYN. Ваша демо компания предварительно одобрена!')}
                disabled={!can('manage_products')}
                title={can('manage_products') ? undefined : denyTitle('manage_products')}
                className="w-full text-left flex items-center justify-between text-sky-700 hover:underline font-semibold disabled:text-gray-300 disabled:no-underline disabled:cursor-not-allowed"
              >
