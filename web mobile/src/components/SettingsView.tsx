...
export default function SettingsView() {
  const { can, denyTitle, role } = useRole();
      ...
      {!canManage && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3.5 flex items-start gap-2.5 text-xs text-amber-900">
          ...
          <div className="space-y-1">
            <span className="font-bold uppercase tracking-wider text-[10px]">Режим только просмотра</span>
            <p className="leading-relaxed">
              Раздел доступен для изменения только Администратору. Сейчас вы вошли как <strong>{role.position}</strong> — поля заблокированы.
            </p>
          </div>
        </div>
      )}
