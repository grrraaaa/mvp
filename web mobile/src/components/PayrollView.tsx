...
interface PayrollViewProps {
  ...
  setEmployees: React.Dispatch<React.SetStateAction<EmployeeSalary[]>>;
}
...
export default function PayrollView({
  ...
  const handleBulkPayrollPayout = (e: React.FormEvent) => {
    e.preventDefault();
    if (!payoutBynAccount) {
    ...
    setTimeout(() => {
      ...
      setAccounts(prev => prev.map(acc => {
        if (acc.id === payoutBynAccount) {
          return { ...acc, balance: Math.max(0, acc.balance - totalPayrollSum) };
        }
        return acc;
      }));

      // Update employee status
      setEmployees(prev => prev.map(e => ({ ...e, status: 'Оплачен' })));

      // Add a bulk bank document record
      const today = new Date();
      const formattedDate = `${String(today.getDate()).padStart(2, '0')}.${String(today.getMonth() + 1).padStart(2, '0')}.${today.getFullYear()}`;
      
      const newInvoiceDoc: BankDocument = {
        id: `№ ${Math.floor(800 + Math.random() * 199)}`,
        date: formattedDate,
        type: 'Зарплатный проект',
        counterparty: `Зарплатный реестр (${employees.length} сотр.)`,
        amount: totalPayrollSum,
        currency: 'BYN',
        status: 'Проведен',
        purpose: 'Выплата заработной платы за выполненные работы по трудовому соглашению'
      };

      setDocuments(prev => [newInvoiceDoc, ...prev]);
