...
const CHART_QUICK_PRESETS: ChartQuickPreset[] = [
  { label: "Остаток на счёте", icon: <Wallet className="w-3.5 h-3.5" />, hint: "График по счетам организации", message: "Сколько на счёте?" },
  { label: "Кассовый прогноз", icon: <TrendingUp className="w-3.5 h-3.5" />, hint: "Прогноз остатка на 30 дней", message: "кассовый прогноз" },
  { label: "Расходы за месяц", icon: <PieChart className="w-3.5 h-3.5" />, hint: "Диаграмма расходов за период", message: "расходы за 2026-05" },
  { label: "Сравнить месяцы", icon: <Scale className="w-3.5 h-3.5" />, hint: "Сравнение февраль и март", message: "сравни февраль и март" },
];
            ...
            >
              ...
              <div className="py-1">
                {CHART_QUICK_PRESETS.map((preset) => (
                  ...
                  >
                    <span className="text-base leading-none mt-0.5 flex-shrink-0 text-[#0d6e68]">{preset.icon}</span>
