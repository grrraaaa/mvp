  ...
  return (
    <div className="banking-app min-h-screen w-full bg-[#f4f6f9] text-[#2c3e50] flex flex-col font-sans relative">
      <Navbar
        ...
        userInitials={
          user?.user_role === "ip" ? "ИП" : user?.user_role === "accountant" ? "БУ" : "ЮЛ"
        }
      ...
      <div className="flex-1 flex flex-col md:flex-row relative">
        ...
        <div className="flex-1 md:pl-[130px] flex flex-col min-w-0 w-full">
          <div className="md:hidden bg-white border-b border-gray-200 px-4 py-2.5 flex items-center justify-between z-10">
            <button
              onClick={() => setMobileSidebarOpen(true)}
              className="p-1.5 hover:bg-slate-100 rounded text-gray-600 flex items-center gap-2 text-xs font-bold"
            >
              <Menu className="w-5 h-5 text-[#138d8a]" />
              <span>СБЕР МЕНЮ</span>
            </button>
          ...
          <main className="flex-1 p-4 md:p-8 w-full select-none outline-none">
