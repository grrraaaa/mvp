...
PLANET_LABELS: dict[str, str] = {
    ...
    "info_requests": "Запросы выписки, информации",
}
...
RESPONSE_TEMPLATES: dict[str, str] = {
    ...
    "credit": "Кредитные продукты для бизнеса (овердрафт, инвестиционный кредит, линия):{link}",
    ...
    "info_requests": (
        "Запросы выписки, остатков и сведений по счетам организации:\n{link}\n\n"
        "Можно сформировать черновик: остаток по счёту, "
        "выписка за период, ведомость неисполненных обязательств."
    ),
    "tax": (
        "Налоги и обязательные платежи в РБ: ФСЗН, подоходный налог, "
        "НДС, отчётность. Подробнее и актуальные ставки — на nalog.gov.by.\n\n"
        "{link}"
    ),
    "finance": (
        "Финансы организации: остатки, бюджет, кассовые разрывы, расходы по категориям. "
        "Точные ставки и сроки сверяют на nalog.gov.by и minfin.gov.by.\n\n"
        "{link}"
    ),
...
def _build_system_prompt() -> str:
    from services.ai.knowledge_sources import KNOWLEDGE_SOURCES_PROMPT

    return f"""Ты — AI-консультант интернет-банка **СберБизнес** (SBBOL) для юридических лиц и ИП в Беларуси.

Правила:
- Отвечай на русском по делу: банковские операции, расчёты, выписки, зарплата, налоги, отчётность, продукты и сервисы для организаций.
- На вопросы пользователя давай содержательный ответ, а не только ссылку на раздел банка.
- НЕ упоминай сайт для физических лиц (www.sber-bank.by, «СберПрайм», retail-карты, ERIP для частных лиц).
- Для банковских действий указывай раздел демо: **внутренний путь** (например `/payments`) или SBBOL: {SBBOL_PROD}/...
- Основные разделы:
  - Главная: /
  - Расчёты: /payments
  - Выписка: /statement
